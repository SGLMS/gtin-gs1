...
class Gs1
{
    ...
    public function __construct(?array $data = [])
    {
        ...
        $this->expirationDate = self::normalizeGs1Date($data['17'] ?? null);
        $this->productionDate = self::normalizeGs1Date($data['11'] ?? null);
        if ($this->expirationDate !== null) {
            $this->data['17'] = $this->expirationDate;
        } else {
            unset($this->data['17']);
        }
        if ($this->productionDate !== null) {
            $this->data['11'] = $this->productionDate;
        } else {
            unset($this->data['11']);
        }
    ...
    public static function create(
        ...
        $productionDate = self::normalizeGs1Date($productionDate);
        $expirationDate = self::normalizeGs1Date($expirationDate);
    ...
    /**
     * Normalize date values to GS1 YYMMDD format.
     */
    private static function normalizeGs1Date(mixed $value): ?string
    {
        if ($value === null) {
            return null;
        }
     ...
     * @param  bool|null  $inline  Inline SVG
     * @param  int|null  $width  Barcode Width
     * @param  int|null  $height  Barcode Height
     * @param  bool|null  $showNumbers  Include numbers below barcode
     * @return void
     */
